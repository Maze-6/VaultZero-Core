...
if 'breach_logged'  not in st.session_state: st.session_state['breach_logged']  = False
...
# FIX: Log the breach event exactly once per breach detection session,
# not every page reload. Uses breach_logged flag in session state.
if is_compromised and not st.session_state['breach_logged']:
    log_audit("SECURITY", "🚨 BREACH_DETECTED", "Honeypot file tampered — Node Alpha integrity compromised.")
    st.session_state['breach_logged'] = True

# Reset the breach_logged flag when system is restored so next breach logs again
if not is_compromised and st.session_state['breach_logged']:
    st.session_state['breach_logged'] = False
...
def lock_after_download():
    """
    Called on_click of Download button.
    - Clears decrypted_file so the download button disappears (forces re-auth next time).
    - Keeps decrypted_name so the Burn button remains visible after download.
    - Logs the download event separately from the reassemble event.
    """
    filename = st.session_state.get('decrypted_name', 'unknown')
    log_audit("CLIENT", "⬇️ FILE_DOWNLOAD", f"Asset '{filename}' downloaded — session lock engaged.")
    st.session_state['decrypted_file'] = None
    # decrypted_name is intentionally NOT cleared here — Burn button needs it
    ...
    with col_down:
            ...
            if st.form_submit_button("🔓 Reassemble"):
                if sel and dk:
                    try:
                        ...
                        k_clean = k_recon.rstrip('\x00').strip()
                        if k_clean == dk.strip():
                            data = b""
                            for i in range(3):
                            ...
                            st.session_state['decrypted_file'] = CryptoEngine.decrypt_payload(k_clean, data)
                            st.session_state['decrypted_name'] = sel
                            log_audit("CLIENT", "🔑 REASSEMBLE", f"Authentication successful for asset '{sel}'.")
                            st.success("Data Reconstruction Successful")
                        else:
                            log_audit("CLIENT", "🚫 ACCESS_DENIED", f"Wrong key supplied for asset '{sel}'.")
                            st.error("Access Denied")
                    except Exception as e: st.error(f"Error: {e}")

        # Download button: on_click clears decrypted_file (forces re-auth next download)
        # but keeps decrypted_name so Burn button stays visible after downloading.
        if st.session_state.get('decrypted_file'):
            st.download_button(
                ...
                on_click=lock_after_download
            )

        # Burn button stays visible as long as decrypted_name is set —
        # this includes after download (where file is cleared but name is kept).
        if st.session_state.get('decrypted_name'):
